...
final class FeatureTargetProducts {
    ...
    private float[] m_products;

    private boolean[] m_alreadyCalculated;

    /**
     *
     */
    FeatureTargetProducts(final int numFeatures) {
        m_products = new float[numFeatures];
        m_alreadyCalculated = new boolean[numFeatures];
    }

    float getFeatureTargetProduct(final int featureIdx, final DataIterator iter) {
        if (m_alreadyCalculated[featureIdx]) {
            return m_products[featureIdx];
        }
        final float product = calculateWeightedInnerProduct(iter);
