...
import org.knime.base.node.mine.regression.glmnet.Feature.FeatureIterator;
...
class Data {
     ...
     *
     */
    Data(final Feature[] features, final float[] target, final WeightContainer weights) {
    ...
    float getWeightedStdv(final int featureIdx) {
        return m_weightedStdv[featureIdx];
    }

    float getWeightedSquaredMean(final int featureIdx) {
        return m_weightedSquaredMeans[featureIdx];
    }

    DataIterator getIterator(final int featureIdx) {
        return new DefaultDataIterator(m_features[featureIdx].getIterator(), m_weightedScaledMeans[featureIdx]);
    }

    float getWeightedMean(final int featureIdx) {
